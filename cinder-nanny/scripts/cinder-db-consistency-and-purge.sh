...
echo "INFO: starting the nanny job for the cinder db consistency check and purge"
if [ "$CINDER_CONSISTENCY_ENABLED" = "True" ] || [ "$CINDER_CONSISTENCY_ENABLED" = "true" ]; then
    if [ "$CINDER_CONSISTENCY_DRY_RUN" = "False" ] || [ "$CINDER_CONSISTENCY_DRY_RUN" = "false" ]; then
        ...
        date
        /var/lib/openstack/bin/python /var/lib/openstack/scripts/cinder-consistency.py --config "${DB_CONFIG}" $FIX_LIMIT
    else
        echo -n "INFO: checking cinder db consistency - "
        date
        /var/lib/openstack/bin/python /var/lib/openstack/scripts/cinder-consistency.py --config "${DB_CONFIG}" --dry-run
    fi
fi
