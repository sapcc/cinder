...
echo "INFO: starting the nanny jobs for the cinder db"
if [ "$CINDER_QUOTA_SYNC_ENABLED" = "True" ] || [ "$CINDER_QUOTA_SYNC_ENABLED" = "true" ]; then
    ...
    for i in `/var/lib/openstack/bin/python /var/lib/openstack/scripts/cinder-quota-sync.py --config "${DB_CONFIG}" --list_projects`; do
        echo project: $i
        /var/lib/openstack/bin/python /var/lib/openstack/scripts/cinder-quota-sync.py --config "${DB_CONFIG}" $SYNC_MODE --project_id $i
    done
fi
