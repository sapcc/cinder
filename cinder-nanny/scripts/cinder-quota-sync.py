...
#    under the License.
#
# Author: Arne Wiebalck <Arne.Wiebalck@cern.ch>  # noqa: H105
...
def get_projects(meta):
    ...
    quota_usages_q = select(columns=[quota_usages_t.c.project_id]).group_by(
...
def yn_choice():
    ...
    print("Do you want to sync? [Yes/No]")  # noqa: C303
    while True:
        choice = input().lower()
        if choice in yes:
            return True
...
def sync_quota_usages_project(meta, project_id, quota_usages_to_sync):
    ...
    print(("Syncing %s", project_id))  # noqa: C303
    now = datetime.datetime.utcnow()
    quota_usages_t = Table("quota_usages", meta, autoload=True)
    for resource, quota in iter(list(quota_usages_to_sync.items())):
        quota_usages_t.update().where(
            and_(
                quota_usages_t.c.project_id == project_id,
                quota_usages_t.c.resource == resource,
            )
        ).values(updated_at=now, in_use=quota).execute()
...
def get_volume_usages_project(meta, project_id):
    ...
    volumes_q = select(
        columns=[volumes_t.c.id, volumes_t.c.size, volumes_t.c.volume_type_id],
        whereclause=and_(
            volumes_t.c.deleted == false(),
...
def get_quota_usages_project(meta, project_id):
    ...
    quota_usages_t = Table("quota_usages", meta, autoload=True)
    quota_usages_q = select(
        columns=[quota_usages_t.c.resource, quota_usages_t.c.in_use],
        whereclause=and_(
            quota_usages_t.c.deleted == false(),
            quota_usages_t.c.project_id == project_id,
        ),
    )
...
def get_resource_types(meta, project_id):
    ...
    types = []
    quota_usages_t = Table("quota_usages", meta, autoload=True)
    resource_types_q = select(
        columns=[quota_usages_t.c.resource, func.count()],
...
def get_volume_types(meta, project_id):
    ...
    volume_types_q = select(
        columns=[volume_types_t.c.id, volume_types_t.c.name],
...
def get_db_url(config_file):
    ...
    try:
        ...
        print("ERROR: Check Cinder configuration file.")  # noqa: C303
        sys.exit(2)
    return db_url
...
def parse_cmdline_args():
    ...
    group.add_argument("--project_id", type=str, help="project to check")
...
def main():
    ...
    volume_types = get_volume_types(cinder_metadata, args.project_id)

    # get the resource types
    resource_types = get_resource_types(cinder_metadata, args.project_id)
    ...
    #
    print(("Checking " + args.project_id + " ..."))  # noqa: C303

    # get the quota usage of a project
    quota_usages = {}
    for resource, count in get_quota_usages_project(
        cinder_metadata, args.project_id
    ):
    ...
    for _, size, type_id in get_volume_usages_project(
        cinder_metadata, args.project_id
    ):
    ...
    for _, size, type_id in get_snapshot_usages_project(
        cinder_metadata, args.project_id
    ):
    ...
    # prepare the output
    ptable = PrettyTable(
        ["Project ID", "Resource", "Quota -> Real", "Sync Status"]
    )
